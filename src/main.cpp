...
const string strMessageMagic = "Ticketcoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, bool verify, uint256 blockHash, int nHeight)
{
    ...
    if(verify && blockHash != uint256("0"))
    {
        std::string cseed_str = blockHash.ToString().substr(7,7);
        ...
        //if lottomining not set and no verification then skip this and stick with original reward
        if(blockHash == uint256("0") && !verify && GetBoolArg("-lottomining", false) )
        {
            nSubsidy*=100;
        }
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, bool verify, uint256 blockHash)
{
...
static const int64_t nTargetTimespanPoW = 1 * 60;  // 1 min this needs to be spacing?
//
...
unsigned int static Digishield(const CBlockIndex* pindexLast)
{
    ...
    int64_t nInterval = nTargetTimespanPoW / nTargetSpacing;
    int64_t retargetTimespan = nTargetTimespanPoW;
    ...
    // thanks to RealSolid & WDC for the basis of this code, modified by Mammix2
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfWork())
    {
        int64_t nReward = GetProofOfWorkReward(nFees, true, GetHash(), pindex->nHeight);
    ...
    if (IsProofOfStake())
    {
        ...
        int64_t nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nFees, true, GetHash());
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Ticketcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Ticketcoin the first lottery mining coin";
        CTransaction txNew;
...
unsigned char pchMessageStart[4] = { 0xa7, 0xb0, 0xa1, 0xc3 };
